...
async function initRoles() {
    ...
    `);
        ...
        const [roleColumn] = await database_1.sequelize.query(`
      SELECT column_name 
      FROM information_schema.columns 
    ...
    `);
      ...
      `);
        ...
        if (userCount > 0) {
            const usersWithRoles = await database_1.sequelize.query(`
      ...
      `);
        ...
        for (const role of roles) {
            const userCount = await User_1.User.count({ where: { roleId: role.id } });
