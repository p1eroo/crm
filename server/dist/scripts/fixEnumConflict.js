...
async function fixEnumConflict() {
    try {
        ...
        const [enums] = await database_1.sequelize.query(`
    ...
    `);
        ...
        const [columns] = await database_1.sequelize.query(`
      SELECT 
        table_name,
        column_name,
        udt_name,
    ...
    `);
        ...
        for (const table of ['companies', 'contacts']) {
            ...
            `);
                        ...
                        try {
                            await database_1.sequelize.query(`DROP TYPE IF EXISTS ${currentType} CASCADE;`);
    ...
    `);
        ...
        const [finalColumns] = await database_1.sequelize.query(`
      SELECT 
        table_name,
        column_name,
        udt_name,
