...
const cleanCompany = (company, includeSensitive = false) => {
    ...
    if (companyData.rubro != null)
        cleaned.rubro = companyData.rubro;
    if (companyData.companyType != null)
        cleaned.companyType = companyData.companyType;
...
router.get('/:id/contacts', async (req, res) => {
    try {
        const company = await Company_1.Company.findByPk(req.params.id, {
            include: [{ model: Contact_1.Contact, as: 'Contacts', attributes: ['id', 'firstName', 'lastName'] }],
        });
        if (!company) {
        ...
        manyToManyContacts.forEach((c) => contactsMap.set(c.id, { id: c.id, firstName: c.firstName ?? '', lastName: c.lastName ?? '' }));
        contactsByCompanyId.forEach((c) => {
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const firstDeal = await Deal_1.Deal.findOne({
            where: { companyId: req.params.id },
            order: [['id', 'ASC']],
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (dealOwnerId != null) {
            try {
                const existingDeals = await Deal_1.Deal.findAll({
                ...
                const dealPayload = { name: dealName, stage, amount, ownerId: dealOwnerId };
                if (dealCloseDate != null)
                    dealPayload.closeDate = dealCloseDate;
...
router.post('/:id/logo', async (req, res) => {
    try {
        ...
        const updated = await Company_1.Company.findByPk(company.id, {
            include: [{ model: User_1.User, as: 'Owner', attributes: ['id', 'firstName', 'lastName'] }],
        });
