// client/src/components/DetailCards/LinkedCompaniesCard.tsx
...
const LinkedCompaniesCard: React.FC<LinkedCompaniesCardProps> = ({ 
  companies, 
  maxItems = 5 
}) => {
  ...
  const totalPages = Math.ceil(companies.length / maxItems);
      ...
      <CardContent>
        ...
        {companies.length > 0 ? (
          <>
            ...
            {/* Paginación - Solo se muestra si hay más de maxItems empresas */}
            {companies.length > maxItems && (
              <Box 
                sx={{ 
                  display: 'flex', 
                  alignItems: 'center', 
...
export default LinkedCompaniesCard;