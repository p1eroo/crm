...
const TaskDetail: React.FC = () => {
  ...
  const [moreMenuAnchorEl, setMoreMenuAnchorEl] = useState<HTMLElement | null>(
    null
  );
  ...
  const [selectedExistingContact, setSelectedExistingContact] = useState<
    number | null
  >(null);
  const [selectedExistingCompany, setSelectedExistingCompany] = useState<
    number | null
  >(null);
  const [selectedExistingDeal, setSelectedExistingDeal] = useState<
    number | null
  >(null);
      ...
      try {
        ...
        setFormData({
          ...
          assignedToId: response.data.assignedToId
            ? response.data.assignedToId.toString()
            : user?.id
            ? user.id.toString()
            : "",
        });
        ...
        if (response.data.dealId) {
          try {
            ...
            setActivities(
              activitiesRes.data.activities || activitiesRes.data || []
            );
          } catch (err) {
        ...
        if (response.data.contactId) {
          try {
            const contactRes = await api.get(
              `/contacts/${response.data.contactId}`
            );
        ...
        if (error.response?.status === 404) {
          ...
          setFormData({
            ...
            assignedToId: activity.assignedToId
              ? activity.assignedToId.toString()
              : user?.id
              ? user.id.toString()
              : "",
          });
  ...
  const fetchActivityLogs = useCallback(async () => {
    ...
    try {
      ...
      const [activitiesResponse, taskResponse] = await Promise.all([
        api
          .get("/activities", { params: { taskId: id } })
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (
        task?.type &&
        ["note", "email", "call", "meeting"].includes(task.type)
      ) {
  ...
  const handleCreateContact = async () => {
    ...
    try {
      ...
      if (!primaryCompanyId) {
        alert(
          "La tarea debe tener al menos una empresa asociada para crear un contacto"
        );
        setSaving(false);
        return;
      }
    ...
    try {
      ...
      const dealData: any = {
        ...
        amount: dealFormData.amount
          ? parseFloat(dealFormData.amount)
          : undefined,
  ...
  const handleSearchRuc = async () => {
    ...
    try {
      ...
      if (response.data.success && response.data.data) {
        ...
        // Capitalizar solo las iniciales
        const nombreCapitalizado = capitalizeInitials(
          data.nombre_o_razon_social || ""
        );
        const direccionCapitalizada = capitalizeInitials(
  ...
  const handleCreateActivity = (
    ...
    switch (type) {
      ...
      case "meeting":
        ...
        setTaskOpen(true);
        // Aquí podrías agregar lógica adicional para diferenciar entre task y meeting
        break;
      case "email":
        // Implementar lógica para email si es necesario
        break;
    }
  };
        ...
        >
          ...
          >
            ...
            >
              ...
              <Chip
                icon={
                  <Flag sx={{ fontSize: 14, color: priorityStyle.color }} />
                }
          ...
          {activeTab === 0 && (
            <>
              ...
              {/* Preparar datos combinados para los cards */}
              {(() => {
                // Combinar contactos
                const allContacts = [];
                if (task.Contact) {
                ...
                return (
                  <>
                    ...
                    >
                      ...
                      <LinkedContactsCard contacts={allContacts} />
                      <LinkedCompaniesCard companies={allCompanies} />
                      <LinkedDealsCard deals={allDeals} />
                    </Box>
                  </>
                );
              })()}
              ...
              {(() => {
                ...
                const allContacts = task.Contact
                  ? [
                      task.Contact,
                      ...taskContacts.filter(
                        (contact: any) => contact.id !== task.Contact?.id
                      ),
                    ]
                  : taskContacts;
                ...
                const allCompanies = task.Company
                  ? [
                      task.Company,
                      ...taskCompanies.filter(
                        (company: any) => company.id !== task.Company?.id
                      ),
                    ]
                  : taskCompanies;
                ...
                return (
                  ...
                  >
                    ...
                    <FullCompaniesTableCard
                      ...
                      getCompanyInitials={(name: string) => {
                        return name
                          ? name.split(" ").length >= 2
                            ? `${name.split(" ")[0][0]}${
                                name.split(" ")[1][0]
          ...
          {activeTab === 2 && (
            <>
              <ActivitiesTabContent
                ...
                emptyMessage="No hay actividades registradas para esta tarea. Crea una nueva actividad para comenzar."
              />
            </>
          )}
        </Box>
                  ...
                  <Box sx={{ display: "flex", gap: 1.5, mb: 1.5 }}>
                    ...
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      ...
                      <Typography
                        ...
                        sx={{ fontSize: "0.8125rem", lineHeight: 1.5 }}
                      >
                        La fecha de vencimiento ya pasó. Revisa el estado de la
                        tarea.
                      </Typography>
                    </Box>
                  </Box>
                      ...
                      <Box sx={{ display: "flex", gap: 1.5, mb: 1.5 }}>
                        ...
                        <Box sx={{ flex: 1, minWidth: 0 }}>
                          ...
                          <Typography
                            ...
                            sx={{ fontSize: "0.8125rem", lineHeight: 1.5 }}
                          >
                            Última actividad hace {daysSinceUpdate} días. Es
                            momento de hacer seguimiento.
                          </Typography>
                        </Box>
                      </Box>
                      <Button
                    ...
                    .map((contact: any) => (
                      <Box
                        ...
                        sx={{
                          ...
                          cursor: "pointer",
                          bgcolor:
                            selectedExistingContact === contact.id
                              ? theme.palette.mode === "dark"
                                ? "rgba(46, 125, 50, 0.2)"
                                : "rgba(46, 125, 50, 0.1)"
                              : "transparent",
          ...
          {dealDialogTab === "create" ? (
            ...
            >
              ...
              <TextField
                ...
                onChange={(e) =>
                  setDealFormData({ ...dealFormData, priority: e.target.value })
                }
                InputLabelProps={{ shrink: true }}
                fullWidth
                sx={{
        ...
        <DialogActions sx={pageStyles.dialogActions}>
          ...
          <Button
            onClick={
              dealDialogTab === "create"
                ? handleCreateDeal
                : handleAddExistingDeal
            }
            variant="contained"
            disabled={
              saving ||
              (dealDialogTab === "existing" && !selectedExistingDeal) ||
              (dealDialogTab === "create" &&
          ...
          <Box sx={{ display: "flex", gap: 1.5, mb: 1.5 }}>
            ...
            <Box sx={{ flex: 1 }}>
              ...
              <TextField
                value={
                  newTaskData.dueDate
                    ? formatDateDisplay(newTaskData.dueDate)
                    : ""
                }
